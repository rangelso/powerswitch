<?php

	define('GPIO_STATE_FILE_PATH', HTML_PATH.'gpiostate');
	define('GPIO_APP_FILE_PATH', HTML_PATH.'power-switch-x4');
	define('GPIO_STATE_FILE_SIZE', 1);
	define('GPIO_ON_COMMAND', 'turnon');
	define('GPIO_OFF_COMMAND', 'turnoff');
	define ('PIN1_BIT',1);
	define ('PIN2_BIT',2);
	define ('PIN3_BIT',3);
	define ('PIN4_BIT',4);
	define ('CHANNEL1_LABEL','TV<br>TUNER<br>X-BOX');
	define ('CHANNEL2_LABEL','PC<BR>MONITOR');
	define ('CHANNEL3_LABEL','FOTEL LEWA');
	define ('CHANNEL4_LABEL','FOTEL PRAWA');
	define ('ALL_ON_LABEL','WŁĄCZ<br>WSZYSTKIE');
	define ('ALL_OFF_LABEL','WYŁĄCZ<br>WSZYSTKIE');
	define ('SHUTDOWN_LABEL','WYŁĄCZ<br>LISTWĘ');
	define ('REBOOT_LABEL','ZRESTARTUJ<br>LISTWĘ');
	define ('REFRESH_LABEL','ODŚWIERZ');
	define ('CELL_HEIGHT',200);	
	define ('FONT_SIZE','7');	
	define ('FONT_FACE','Arial Black');
	define ('TABLE_BG_COL','#c7dbf9');
	define ('TABLE_BORDER_COL','#c7dbf9');
	define ('BG_ON_COL','#f9c57c');
	define ('BG_OFF_COL','#c7dbf9');
	
?>
